...
<div id="pageAll">
                    ...
                    <c:forEach items="${userPageInfo.list}" var="friend">
                        <tr height="40px" id="tr_${friend.id}">
                            ...
                            <td>
                                ...
                                <input type="button" id="resetPassword_${friend.id}" data-id="${friend.id}" value="重置密码">


                                <input type="button" id="disable_${friend.id}" data-id="${friend.id}" value="禁用用户">
    ...
    <ul id="PageNum">
        <li><a href="${path}/manage/user/list?pageNum=1">首页</a></li>
        <li><a href="${path}/manage/user/list?pageNum=${userPageInfo.prePage}">上一页</a></li>
        <c:forEach items="${userPageInfo.navigatepageNums}" var="i">
            <li><a href="${path}/manage/user/list?pageNum=${i}">${i}</a></li>
        </c:forEach>
        <li><a href="${path}/manage/user/list?pageNum=${userPageInfo.nextPage}">下一页</a></li>
        <li><a href="${path}/manage/user/list?pageNum=${userPageInfo.pages}">尾页</a></li>
...
<script type="text/javascript">
    ...
    //用户重置密码
    $("input[id^='resetPassword']").click(function (){
        var id = $(this).data("id");
        console.log(id);
        $.ajax({
            url: "${path}/manage/user/resetPassword",
            data: {id: id},
            type: "GET",
            async: false,
            success: function(data) {
                console.log(data);
                alert("密码重置成功，用户的新密码为[" + data + "]，请告知用户！");
            },
    ...
    $("input[id^='disable']").click(function (){
        var id = $(this).data("id");
        var element = $(this);
        console.log(id);
        $.ajax({
            url: "${path}/manage/user/disable",
            data: {id: id},
            type: "GET",
            success: function(data) {
                element.parents("tr").remove();
                alert("用户禁用成功！请在用户黑名单里查看用户！");
            }
        });
    });
</script>